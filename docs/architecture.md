...
- `courts_searched` is reported by `CourtSearchReport`, once per search, on the first value the repository produces — which is the complete answer, because `CompositeCourtRepository` holds that first value back until every source has replied (see below). Anything after it is a live update to a search already counted. Counting every value instead would double every number in the dashboard. The position fix sits inside the reported span: to the user, a search that never ran because the app couldn't locate them is a search that failed.
...
- `CompositeCourtRepository` receives a `List<CourtRepository>` (Overpass then Firestore, wired in `court_repository_provider.dart`) and itself implements `CourtRepository`:
  ...
  - `watchCourtsInBounds` holds its **first** value back until every source has had its say — a value, an error, or the end of its stream. The sources answer at very different speeds: Firestore replies almost at once and almost always with nothing, since few courts are added by hand, while OpenStreetMap takes a second or more and holds nearly all of them. Emitting each reply as it arrived meant a search answered "no courts here yet" and then corrected itself — and because that first answer reaches the UI, the list rendered its entire empty state, inviting the user to map an area that already had forty courts in it. A failing or finished source counts as having answered, so one dead source cannot hold a search open forever. After that first complete answer, later values flow straight through: those are live updates, not a search answering twice. This is also what lets `courts_searched` simply count the first value;
